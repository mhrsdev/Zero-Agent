...
async def test_viewer_can_read_but_cannot_mutate(panel):
    ...
    code=next(re.search(r'\b(\d{6})\b',text).group(1) for _,text in bot.messages if 'کد ورود' in text)
    ...
    assert '@ysnrfd3' in alert and '203.0.113.7' in alert and 'زمان:' in alert
    assert (await client.get('/api/dashboard',headers=headers)).status==200
