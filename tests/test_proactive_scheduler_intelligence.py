...
def test_cancellation_propagates_and_stops_outbox(tmp_path):
 s,q=setup(tmp_path);add(s,'parent',-1,1);add(s,'child',-1,1,parent='parent',status='retryable_failed');q.outbox.reserve('child','w',NOW,installation_id='inst-test',group_id='group-test');q.cancel('parent','user_opt_out',NOW)
 with s._conn() as c:
