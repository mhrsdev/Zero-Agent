...
def test_outbox_reservation_is_atomic_and_recovery_is_ambiguous(tmp_path):
 s=ZeroStore(str(tmp_path/'x.db'));o=Outbox(s);assert o.reserve('c','a',100,installation_id='inst-test',group_id='group-test')== 'pf:c';assert o.reserve('c','b',101,installation_id='inst-test',group_id='group-test') is None;o.recover(1001)
 with s._conn() as c:assert c.execute("select send_state from proactive_followup_outbox").fetchone()[0]=='ambiguous'
 assert o.reserve('c','b',1002,installation_id='inst-test',group_id='group-test') is None

def test_outbox_receipt_persists_after_reopen(tmp_path):
 path=str(tmp_path/'x.db');s=ZeroStore(path);o=Outbox(s);key=o.reserve('c','a',100,installation_id='inst-test',group_id='group-test');o.complete(key,TransportResult(True,'42'))
