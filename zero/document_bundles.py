...
class DocumentBundles:
 ...
 def _open(self,chat,sender):
  with self.store._conn() as c:return c.execute("select * from group_document_bundles where chat_id=? and sender_id=? and status='open' order by updated_at desc limit 1",(chat,sender)).fetchone()
 async def observe(self,message):
  ...
  if open_row and reference:
   with self.store._conn() as c:c.execute("update group_document_bundles set status='sealed',sealed_at=?,completion_reason='reference',updated_at=?,version=version+1 where bundle_id=?",(now,now,open_row['bundle_id']))
  ...
  if open_row and now-int(open_row['updated_at'])<=600: bid=open_row['bundle_id']
   ...
   bid=str(uuid.uuid4())
   with self.store._conn() as c:c.execute("insert into group_document_bundles(bundle_id,chat_id,sender_id,status,content_type,started_at,updated_at) values(?,?,?,?,?,?,?)",(bid,message.chat_id,message.sender_id,'open','unknown',now,now))
 ...
 def active_part_ids(self,chat):
  with self.store._conn() as c:return {int(r[0]) for r in c.execute("select p.telegram_message_id from group_document_bundle_parts p join group_document_bundles b on b.bundle_id=p.bundle_id where b.chat_id=? and b.status in ('open','sealed')",(chat,)).fetchall()}
 def live_entries(self,chat):
  with self.store._conn() as c:return [f"DOCUMENT BUNDLE type={r['content_type']} parts={r['part_count']} status={r['status']}" for r in c.execute("select * from group_document_bundles where chat_id=? and status in ('open','sealed') order by updated_at desc limit 3",(chat,)).fetchall()]
