...
class GroupContext:
    ...
    async def build(self,message, suppressed_ids:set[int]|None=None):
        suppressed_ids=suppressed_ids or set(); state=await self.store.get_group_context_state(message.chat_id); rows=await self.store.get_unconsumed_group_messages(message.chat_id,60)
        ...
        committed=await self.store.commit_group_context(message.chat_id,commit_rows,summary,int(state['optimistic_version']))
