...
def parse(text:str, available:set[str])->RetrievalPlan|None:
    ...
    actors=tuple(x.strip()[:64] for x in raw.get('actors',[]) if isinstance(x,str) and x.strip() and not re.search(r'\d{7,}|[/\\\\]|select|sqlite',x,re.I))[:4]; subjects=tuple(x.strip()[:64] for x in raw.get('subjects',[]) if isinstance(x,str) and x.strip() and not re.search(r'\d{7,}|[/\\\\]|select|sqlite',x,re.I))[:4]
    if op not in OPS or mode not in EVIDENCE or tk not in TIMES: return None
    # unresolved references are intentionally omitted; never become self.
...
def window(kind:str, now:float|None=None):
    ...
    elif kind=='last_night': start=(local.replace(hour=0,minute=0,second=0,microsecond=0)-timedelta(days=1)); return start.timestamp(),local.replace(hour=6,minute=0,second=0,microsecond=0).timestamp()
