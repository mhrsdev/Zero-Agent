...
def _source_rows(source: Path) -> list[sqlite3.Row]:
    with sqlite3.connect(source) as db:
        db.row_factory = sqlite3.Row
        tables = {r[0] for r in db.execute("SELECT name FROM sqlite_master WHERE type='table'")}
        if "long_term_memory" not in tables:
            return []
        columns = {r[1] for r in db.execute("PRAGMA table_info(long_term_memory)")}
        required = {"memory_id", "chat_id", "subject_user_id", "category", "content", "confidence", "source_message_ids_json", "created_at", "expires_at", "status"}
        missing = required - columns
        if missing:
            raise ValueError(f"unsupported V1 long_term_memory schema: {sorted(missing)}")
        return list(db.execute("SELECT * FROM long_term_memory WHERE status IN ('active','verified')"))
...
def _item(row: sqlite3.Row) -> MemoryV3Item:
...
def apply_v1_to_v3(
    ...
    *,
    ...
    for row in rows:
        source_id, row_hash = str(row["memory_id"]), hashlib.sha256(repr(tuple(row)).encode()).hexdigest()
        ...
        with sqlite3.connect(target) as db:
            prior = db.execute("SELECT action FROM memory_v3_migration_map WHERE run_id=? AND source_table=? AND source_id=? AND source_sha256=?", (run_id, "long_term_memory", source_id, row_hash)).fetchone()
        if prior:
        ...
        try:
            ...
            quarantined += 1
            with sqlite3.connect(target) as db:
                db.execute("INSERT OR IGNORE INTO memory_v3_quarantine(source_name,source_id,reason,created_at) VALUES(?,?,?,?)", (f"v1:{source.resolve()}", f"long_term_memory:{source_id}", str(exc), time.time()))
                db.execute("INSERT OR IGNORE INTO memory_v3_migration_map VALUES(?,?,?,?,?,?)", (run_id, "long_term_memory", source_id, row_hash, None, "quarantine"))
            continue
        if dry_run:
            continue
        ...
        with sqlite3.connect(target) as db:
            db.execute("INSERT OR IGNORE INTO memory_v3_migration_map VALUES(?,?,?,?,?,?)", (run_id, "long_term_memory", source_id, row_hash, target_id, action))
        if action == "reused": reused += 1
        else: imported += 1
    with sqlite3.connect(target) as db:
    ...
    return MigrationResult(run_id, len(rows), imported, reused, quarantined)
