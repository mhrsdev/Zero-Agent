...
class OfficeRepository:
    ...
    def reserve_and_create(
        ...
        with self.connect() as conn:
            ...
            if not unlimited:
                conn.execute(
                    "INSERT INTO office_quota_usage(user_id,quota_date,updated_at) VALUES(?,?,?) ON CONFLICT(user_id,quota_date) DO NOTHING",
                    (user_id, quota_date, now),
                )
                usage = conn.execute(
                    "SELECT * FROM office_quota_usage WHERE user_id=? AND quota_date=?", (user_id, quota_date)
                ).fetchone()
                if int(usage["jobs_reserved"]) + int(usage["jobs_committed"]) >= jobs_limit:
                    conn.rollback()
                    raise QuotaExceeded("daily_jobs")
                conn.execute(
                    "UPDATE office_quota_usage SET jobs_reserved=jobs_reserved+1,characters_reserved=characters_reserved+?,last_job_id=?,updated_at=? WHERE user_id=? AND quota_date=?",
                    (extracted_characters, job_id, now, user_id, quota_date),
                )
            conn.execute(
                """INSERT INTO office_jobs(
                    id,trace_id,account_scope,user_id,chat_id,message_id,status,operation_type,office_format,request_text,
                ...
                (job_id, trace_id, account_scope, user_id, chat_id, message_id, operation_type, office_format, request_text,
                 input_filename, input_path, detected_mime, input_size_bytes, uncompressed_size_bytes, extracted_characters,
    ...
    def quota_usage(self, user_id: int, quota_date: str) -> dict[str, int]:
    ...
    def _finish_quota(self, job_id: str, *, commit: bool, reason: str) -> bool:
        ...
        with self.connect() as conn:
            conn.execute("BEGIN IMMEDIATE")
            row = conn.execute("SELECT user_id,quota_date,extracted_characters,quota_state,status FROM office_jobs WHERE id=?", (job_id,)).fetchone()
            ...
            if commit:
                conn.execute(
                    ...
                    (chars, chars, now, row["user_id"], row["quota_date"]),
                )
                state, event = "committed", "quota_committed"
            else:
                conn.execute(
                    """UPDATE office_quota_usage SET jobs_reserved=jobs_reserved-1,
                       characters_reserved=characters_reserved-?,updated_at=? WHERE user_id=? AND quota_date=?""",
    ...
    def reserve_delivery(self, job_id: str, worker: str, *, lease_seconds: int, now: int | None = None) -> str | None:
        ...
        with self.connect() as conn:
            ...
            if row:
                ...
                key = f"office:{job_id}"
                conn.execute(
                    "INSERT INTO office_delivery_outbox(id,job_id,outbound_key,status,lease_owner,lease_expires_at,created_at,updated_at) VALUES(?,?,?,'reserved',?,?,?,?)",
                    (str(uuid.uuid4()), job_id, key, worker, now + lease_seconds, now, now),
                )
    ...
    def _commit_quota_locked(conn: sqlite3.Connection, job_id: str, now: int) -> bool:
        row = conn.execute(
            "SELECT user_id,quota_date,extracted_characters,quota_state,status FROM office_jobs WHERE id=?",
            (job_id,),
        ).fetchone()
        if not row or row["quota_state"] != "reserved":
            return False
        ...
        changed = conn.execute(
            """UPDATE office_quota_usage SET jobs_reserved=jobs_reserved-1,jobs_committed=jobs_committed+1,
               characters_reserved=characters_reserved-?,characters_committed=characters_committed+?,updated_at=?
               WHERE user_id=? AND quota_date=? AND jobs_reserved>0""",
            (chars, chars, now, row["user_id"], row["quota_date"]),
        ).rowcount
        if changed != 1:
