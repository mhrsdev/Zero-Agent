...
class DeliveryCoordinator:
    """Scope-aware delivery: verifies the job's installation_id and group_id
    before sending, and refuses to deliver a job whose owner is missing or
    carries a forbidden fallback (legacy, candidate:, 0, default)."""
    ...
    @staticmethod
    def _verify_job_scope(job: dict[str, Any]) -> None:
        """P0-2: fail-closed if the job is missing an explicit owner."""
        from zero.tenancy.models import FORBIDDEN_IDS, _CANDIDATE_PREFIX
        for field_name in ("installation_id", "group_id"):
            value = str(job.get(field_name, "") or "")
            if not value.strip():
                raise RuntimeError(f"job_{field_name}_missing")
            if value.casefold() in FORBIDDEN_IDS:
                raise RuntimeError(f"job_{field_name}_forbidden")
            if value.startswith(_CANDIDATE_PREFIX):
                raise RuntimeError(f"job_{field_name}_candidate")
    ...
    async def tick(self) -> dict[str, Any] | None:
        ...
        for job in jobs:
            ...
            send_returned = False
            try:
                # P0-2: verify explicit ownership before any side effect.
                self._verify_job_scope(job)
                if job["output_path"]:
