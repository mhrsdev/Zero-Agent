...
class PanelAPI:
    ...
    async def _rate_limit(self, request, handler):
        ...
        if len(q)>=limit: return self._json_error('درخواست بیش از حد مجاز است.',429)
    ...
    async def _notify_login(self,request,session):
        ...
        text=(f'🛡️ ورود به پنل Zero\nادمین: {username}\n'
              f'IP: {self._client_ip(request)}\n'
              f'زمان: {datetime.now(timezone.utc).isoformat(timespec="seconds")} UTC')
        try:
    ...
    async def _body(self,request):
        ...
        except Exception: raise web.HTTPBadRequest(text=json.dumps({'error':'درخواست نامعتبر است.'}),content_type='application/json')
    ...
    async def _request_code(self,request):
        ...
        if user_id is None: self._audit('LOGIN_DENIED',None,reason='not_allowed'); return self._json_error('این شناسه اجازه ورود ندارد.',403)
        old=self.pending.get(str(user_id))
        if old and now-old['sent']<30:return self._json_error('لطفاً چند ثانیه صبر کنید.',429)
        code=f'{secrets.randbelow(1_000_000):06d}'; self.pending[str(user_id)]={'hash':self._hash(code),'sent':now,'expires':now+120,'attempts':0}
        await self.bot.send_message(user_id,f'کد ورود امن پنل Zero: {code}\nاعتبار: ۲ دقیقه'); self._audit('OTP_SENT',{'user_id':user_id})
    ...
    async def _verify(self,request):
        ...
        if not item or item['expires']<time.time() or item['attempts']>=5:return self._json_error('کد منقضی شده یا تلاش‌ها تمام شده است.',401)
        item['attempts']+=1
        if not hmac.compare_digest(item['hash'],self._hash(str(data.get('code','')))):return self._json_error('کد واردشده صحیح نیست.',401)
    ...
    async def _chats(self,request):
        ...
        except ValueError:return self._json_error('شناسه نامعتبر است.')
    ...
    async def _memory_action(self,request):
        ...
        try:
            ...
            else:return self._json_error('این عملیات در Backend فعلی پشتیبانی نمی‌شود.',405)
    ...
    async def _knowledge_run(self,request):
        ...
        if body.get('confirm') is not True:return self._json_error('تأیید عملیات لازم است.',409)
    ...
    async def _job_action(self,request):
        ...
        else:return self._json_error('عملیات پشتیبانی نمی‌شود.',405)
    ...
    async def _setting_update(self,request):
        ...
        if key not in _ALLOWED_SETTINGS:return self._json_error('تنظیم اجازه ویرایش ندارد.',403)
        value=body.get('value');
        if not isinstance(value,(bool,int,float,str)):return self._json_error('مقدار نامعتبر است.')
