...
from .tenancy import GROUP_SETTING_KEYS, Role, Scope, TenancyError
...
class PanelAPI:
    ...
    def __init__(self, config, store, router, bot, *, static_dir: str | Path, services: dict[str, Any] | None = None, panel_store: PanelStore | None = None):
        ...
        self.app.add_routes([
            ...
            web.get('/api/usage', self._usage),
            web.get('/api/backups', self._backups),
            web.get('/api/groups', self._groups),
            web.get(r'/api/groups/{chat_id}', self._group_detail),
            web.post(r'/api/groups/{chat_id}/settings', self._group_setting_update),
        ])
    ...
    async def _local_bootstrap(self, request):
        ...
        try:
            ...
            # streams.
            admin_id = await asyncio.to_thread(self.panel_store.create_admin, username, password)
    ...
    def _tenancy(self):
        return self.services.get('tenancy')

    def _installation_id(self) -> str:
        return str(self.services.get('installation_id') or os.environ.get('ZERO_INSTALLATION_ID') or 'local')

    async def _groups(self, request):
        await self._require(request)
        tenancy = self._tenancy()
        if tenancy is None:
            return web.json_response({'items': [], 'setting_keys': sorted(GROUP_SETTING_KEYS)})
        items = []
        for group in tenancy.groups(self._installation_id()):
            scope = Scope(group.installation_id, group.group_id)
            items.append({
                'group_id': group.group_id,
                'chat_id': group.platform_chat_id,
                'title': group.title,
                'state': group.state.value if hasattr(group.state, 'value') else str(group.state),
                'settings': tenancy.settings(scope),
            })
        return web.json_response({'items': items, 'setting_keys': sorted(GROUP_SETTING_KEYS)})

    async def _group_detail(self, request):
        await self._require(request)
        tenancy = self._tenancy()
        if tenancy is None:
            return self._json_error('Tenancy is not configured.', 503)
        try:
            chat_id = int(request.match_info['chat_id'])
            scope = tenancy.resolve_scope(self._installation_id(), platform_chat_id=chat_id, user_id=int(self.config.owner_user_id or 0) or None)
        except (TypeError, ValueError, TenancyError):
            return self._json_error('Group not found.', 404)
        group = tenancy.get_group(scope.installation_id, scope.group_id)
        return web.json_response({
            ...
            'state': group.state.value if hasattr(group.state, 'value') else str(group.state),
            'settings': tenancy.settings(scope),
            'setting_keys': sorted(GROUP_SETTING_KEYS),
        })

    async def _group_setting_update(self, request):
        session = await self._require(request, csrf=True, write=True)
        tenancy = self._tenancy()
        if tenancy is None:
            return self._json_error('Tenancy is not configured.', 503)
        ...
        if key not in GROUP_SETTING_KEYS:
            return self._json_error('Setting is not editable.', 403)
        try:
            chat_id = int(request.match_info['chat_id'])
            owner_id = int(self.config.owner_user_id or 0)
            scope = tenancy.resolve_scope(self._installation_id(), platform_chat_id=chat_id, user_id=owner_id or None)
        except (TypeError, ValueError, TenancyError):
            return self._json_error('Group not found.', 404)
        if owner_id and tenancy.role_of(scope, owner_id) is None:
            tenancy.add_member(scope, owner_id, Role.OWNER)
        try:
            tenancy.set_setting(scope, key, body.get('value'), actor_id=owner_id or None)
        except (ValueError, TenancyError) as exc:
            return self._json_error(str(exc) or 'Unable to save group setting.', 400)
        self._audit('GROUP_SETTING_UPDATED', session, chat_id=chat_id, key=key)
        return web.json_response({'ok': True, 'key': key, 'chat_id': chat_id})
    ...
    async def _usage(self, request):
        ...
        stats = {}
        try:
            stats = await self.store.get_today_stats(day)
        except Exception as exc:
            logger.warning('PANEL_USAGE_FAILED exception_type=%s', type(exc).__name__)
            return web.json_response({'day': day, 'stats': {}, 'state': 'failed', 'reason': type(exc).__name__})
        jobs = []
        worker = self.services.get('jobs')
        if worker is not None:
            try:
                jobs = await worker.list_jobs(actor=int(self.config.owner_user_id))
            except Exception as exc:
                logger.warning('PANEL_USAGE_JOBS_FAILED exception_type=%s', type(exc).__name__)
        return web.json_response({'day': day, 'stats': stats or {}, 'jobs': jobs, 'state': 'measured'})

    async def _backups(self, request):
        await self._require(request)
        root = zero_home_path('backups')
        items = []
        try:
            if root.exists():
                for path in sorted(root.iterdir())[-50:]:
                    if path.is_file():
                        details = path.stat()
                        items.append({'name': path.name, 'size': details.st_size, 'mtime': int(details.st_mtime)})
        except OSError as exc:
            return web.json_response({'items': [], 'state': 'failed', 'reason': type(exc).__name__})
        return web.json_response({'items': items, 'state': 'measured'})
