...
class Outbox:
 ...
 def complete(self,key,result):
  ...
  with sqlite_txn(self.store._conn()) as c:c.execute('update proactive_followup_outbox set send_state=?,receipt=?,last_error=?,lease_until=null,updated_at=? where outbound_key=?',(state,result.receipt,result.error_code,now,key))
 def recover(self,now=None):
  now=now or int(time.time())
  with sqlite_txn(self.store._conn()) as c:c.execute("update proactive_followup_outbox set send_state='ambiguous',lease_until=null,updated_at=? where send_state in ('reserved','sending') and lease_until<?",(now,now))
