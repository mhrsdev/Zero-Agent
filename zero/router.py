...
class IndependentRouter:
    def __init__(self, config: ZeroConfig, *, registry: Any = None):
        self.config = config
        ...
        self.registry = registry  # ProviderRegistry or None (legacy mode)
    ...
    async def complete(self, prompt: str, *, max_output_tokens: int = 700) -> RouteResult:
        ...
        if (not legacy) and self.registry is not None and self.registry.names():
            try:
                ...
                return RouteResult(
                    text=result.text, provider=result.profile, model=result.model,
        ...
        for provider in order[:max_attempts]:
            ...
            if result.text:
                ...
                return result
        return RouteResult(text="Responses are currently unavailable; please try again later.", provider="fallback", model="none", attempts=last.attempts, metadata={"error": last.metadata.get("error", "unavailable"), "fallback_used": True})
